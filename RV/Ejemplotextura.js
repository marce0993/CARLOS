...
TEXTURA.retrollamada = function(textura) {
 ...
 TEXTURA.setup = function() {
  ...
  TEXTURA.camara= new THREE.PerspectiveCamera(75,
                                              window.innnerWidth/window.innerHeigh
                                               0.1,
                                               1000);
  TEXTURA.camara.position.z=5;
  var lienzo = document.getElementById("ejemplo-textura");
  TEXTURA.renderizador = new THREE.WebGLRenderer({canvas: linezo,
                                            antialias: true});
  TEXTURA.renderizador.setSize(600,600);
 }
 TEXTURA.loop=function(){
 requestAnimationFrame(TEXTURA.loop);
 if(TEXTURA.malla !== uindefined){
